...
function auth(req, res, next){
    ...
    try {
        jwt.verify(splitToken[1], 'xyz', function(err, decoded){ //decoded means, change user ID encoded to original ID
            req.user = decoded._id
            //console.log(decoded)
            User.findById (
                req.user, (err, data) => {
                    if (!data) return res.status(404).json(failRes("User Not Found"))
                    //res.status(201).json(sucRes(data, "data ready"))// if use this, can cause crash due to re send to header again, but data sucess create 
                }
            )
            next()
        })
